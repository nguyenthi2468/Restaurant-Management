...
export const useGetKitchenTicketItemsByStatusQuery = (status?: string) => {
  return useQuery({
    queryKey: ['kitchen-ticket-items', status],
    queryFn: () => getKitchenTicketItemsByStatus(status),
    enabled: true,
  });
};
